...
from analytics import log_query, get_stats

app = FastAPI(
    title="LangGraph RAG Agent API",
    description="API for the LangGraph-powered RAG agent with Pinecone and OpenAI.",
    version="1.0.0",
)
    ...
    try:
        ...
        for i, s in enumerate(rag_agent.stream(inputs, config=config)):
            ...
            if current_node_name == "router":
                ...
                if rag_sufficient:
                    event_description = "RAG Lookup performed. Content found and deemed sufficient. Proceeding to answer."
                    event_details = {"retrieved_content_summary": rag_content_summary, "sufficiency_verdict": "Sufficient"}
                else:
                    event_description = "RAG Lookup performed. Content NOT sufficient. Diverting to web search."
            ...
            trace_events_for_frontend.append(
                TraceEvent(step=i + 1, node_name=current_node_name,
                           description=event_description, details=event_details, event_type=event_type)
            )
        ...
        final_route = trace_events_for_frontend[-1].node_name if trace_events_for_frontend else "unknown"
        used_web = any(e.node_name == "web_search" for e in trace_events_for_frontend)
        log_query(request.session_id, request.query, final_route, used_web, response_time)
